...
const backPack = `PPZTzDhJPLqPhqDTqrwQZZWbmCBMJMcsNmCBFWmMcsNb
...
const newPack = backPack.split('\n')
let total = []

newPack.forEach(container => {
    const firstContainer = container.slice(container[0], container.length/2)
    const secondContainer = container.slice(container.length/2)

    for(const letter of firstContainer){  
        if (secondContainer.includes(letter)) {
            total.push(letter)
            break;
        }
    }
...
console.log('sum is: ', getSum(total))

let someArray =[];
 for( let x = 0; x < newPack.length; x += 3) {
    const temp = newPack.slice(x, x+3);
        for (const letter of temp[0]) {
            if(temp[1].includes(letter) && temp[2].includes(letter)) {
                someArray.push(letter);
                break;
            }
    }
 }
...
console.log('sum2 is: ',getSum(someArray))
